...
class TaylorF2:
    ...
    def __init__(self, m1, m2, chi1, chi2, qm_def1=1.0, qm_def2=1.0):
        ...
        self.v[0] = 1.0
        self.v[1] = 0.0
        self.v[2] = 5.0*(743.0/84.0 + 11.0 * eta)/9.0
        self.v[3] = -16.0*np.pi
        self.v[4] = 5.0*(3058.673/7.056 + 5429.0/7.0 * eta + 617.0 * eta*eta)/72.0
        self.v[5] = 5.0/9.0 * (7729.0/84.0 - 13.0 * eta) * np.pi
        self.vlogv[5] = 5.0/3.0 * (7729.0/84.0 - 13.0 * eta) * np.pi
        self.v[6] = (11583.231236531/4.694215680 - 640.0/3.0 * np.pi*np.pi - 6848.0/21.0*EULER_GAMMA) \
                     + eta * (-15737.765635/3.048192 + 2255.0/12.0 * np.pi*np.pi)  \
                     + eta*eta * 76055.0/1728.0 \
                     - eta*eta*eta * 127825.0/1296.0
        self.v[6] += (-6848.0/21.0)*np.log(4.0)
        self.vlogv[6] = -6848.0/21.0
        self.v[7] = np.pi * ( 77096675.0/254016.0 + 378515.0/1512.0 * eta - 74045.0/756.0 * eta*eta)
        ...
        pn_sigma += (720.0*qm_def1 - 1.0)/96.0 * m1M * m1M * chi1 * chi1
        pn_sigma += (720.0*qm_def2 - 1.0)/96.0 * m2M * m2M * chi2 * chi2
        pn_sigma -= (240.0*qm_def1 - 7.0)/96.0 * m1M * m1M * chi1sq
        pn_sigma -= (240.0*qm_def2 - 7.0)/96.0 * m2M * m2M * chi2sq

        pn_ss3 =  (326.75/1.12 + 557.5/1.8*eta) * eta*chi1*chi2
        pn_ss3 += ((4703.5/8.4 + 2935.0/6.0*m1M - 120.0*m1M*m1M) * qm_def1 + (-4108.25/6.72 - 108.5/1.2*m1M + 125.5/3.6*m1M*m1M)) * m1M*m1M * chi1sq
        pn_ss3 += ((4703.5/8.4 + 2935.0/6.0*m2M - 120.0*m2M*m2M) * qm_def2 + (-4108.25/6.72 - 108.5/1.2*m2M + 125.5/3.6*m2M*m2M)) * m2M*m2M * chi2sq
        ...
        self.v[7] += (-8980424995.0/762048.0 + 6586595.0*eta/756.0 - 305.0*eta*eta/36.0)*SL - (170978035.0/48384.0 - 2876425.0*eta/672.0 - 4735.0*eta*eta/144.0) * dSigmaL
        self.v[6] += np.pi * (3760.0*SL + 1490.0*dSigmaL)/3.0 + pn_ss3
        self.v[5] += -1.0 * pn_gamma
        self.vlogv[5] += -3.0 * pn_gamma
        self.v[4] += -10.0 * pn_sigma
        self.v[3] += 188.0*SL/3.0 + 25.0*dSigmaL
        
                
        # multiply all coefficients by pfaN
        for i in np.arange(1 + PN_PHASING_SERIES_MAX_ORDER):
            self.v[i] *= pfaN
            self.vlogv[i] *= pfaN
            self.vlogvsq[i] *= pfaN


    def compute_phasing(self, Mf_array, Mf0=1.0):
        """
        ...
        for Mf in Mf_array:
            ...
            phasing += self.v[7] * v7;
            phasing += (self.v[6] + self.vlogv[6] * logv) * v6;
            phasing += (self.v[5] + self.vlogv[5] * logv) * v5;
            phasing += self.v[4] * v4;
            phasing += self.v[3] * v3;
            phasing += self.v[2] * v2;
            phasing += self.v[1] * v;
    ...
    def compute_strain(self, Mf, Mf0=1.0, A0=1.0):
        """
        ...
        return A0 * (Mf/Mf0)**(-7.0/6.0) * np.exp(1j * phi)